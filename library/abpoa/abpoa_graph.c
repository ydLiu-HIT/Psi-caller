...
void abpoa_diploid_heaviest_column(abpoa_graph_t *graph, int src_id, int sink_id, int seq_n, double min_freq, FILE *out_fp, uint8_t ***cons_seq, int **cons_l, int *cons_n, int *cluster_n) {
    abpoa_set_msa_rank(graph, src_id, sink_id);
    ...
    //****
    if(clu_n > 1)
        for(int t = 0; t < clu_n; ++t)
            cluster_n[t] = clu_read_ids_n[t];
...
int abpoa_generate_consensus(abpoa_t *ab, abpoa_para_t *abpt, int seq_n, FILE *out_fp, uint8_t ***cons_seq, int **cons_l, int *cons_n, int *cluster_n) {
    abpoa_graph_t *graph = ab->abg;
    ...
    if (abpt->is_diploid) {
        abpoa_diploid_heaviest_column(graph, ABPOA_SRC_NODE_ID, ABPOA_SINK_NODE_ID, seq_n, abpt->min_freq, out_fp, cons_seq, cons_l, cons_n, cluster_n);
    } else {
