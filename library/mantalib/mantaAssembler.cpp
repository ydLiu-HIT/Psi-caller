...
static PyObject* mantaAss(PyObject *self, PyObject *args) 
{
    PyObject *listObj;
    unsigned minWordLength;
    unsigned maxWordLength;
    if(! PyArg_ParseTuple(args, "Oii", &listObj, &minWordLength, &maxWordLength))
        return NULL;
    //length of the list
    unsigned seq_n = PyList_Size(listObj);
    //iterate over all the element
    AssemblyManager *am_ = new AssemblyManager[1];
	AssemblyManager &am = *am_;

    am.o.minWordLength = minWordLength;
    am.o.maxWordLength = maxWordLength;

    for(unsigned i = 0; i < seq_n; ++i)
    {
        am.reads.emplace_back(PyBytes_AsString(PyList_GetItem(listObj, i)));
    }
    Py_CLEAR(listObj);  //free

    //printf("%d, %d\n", am.o.minWordLength, am.o.maxWordLength);
    am.assembley();

	auto &contigs = am.getResults();
    
    unsigned cons_n = contigs.size();
    PyObject *rList_cons = PyList_New( cons_n );
    PyObject *rList_n = PyList_New( cons_n );
    
    unsigned cnt = 0;
	for(auto &contig:contigs){
		//contig.debug_print(std::cerr);
        //const char *p = contig.getSeq();
        //unsigned c = contig.getSupportReadCnt();
        //printf("%s\n%d\n", p, c);

        PyList_SetItem(rList_cons, cnt, PyBytes_FromString(contig.getSeq()));
        PyList_SetItem(rList_n, cnt, PyLong_FromLong(contig.getSupportReadCnt()));
        cnt += 1;
	}
...
static PyMethodDef myMethods[] = {
    {"mantaAss", mantaAss, METH_VARARGS, "Execute other function."},
    {NULL, NULL, 0, NULL}
};

/*module initialization */
/*Python version 3 */
static struct PyModuleDef mantaAssModuleDem = 
{
    PyModuleDef_HEAD_INIT,
    "mantaAss_module", "Some documentation",
    -1,
    myMethods
};
PyMODINIT_FUNC PyInit_mantaAss_module(void)
{
    return PyModule_Create(&mantaAssModuleDem);
}
